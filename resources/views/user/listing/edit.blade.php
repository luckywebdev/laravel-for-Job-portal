...
<script>
    ...
    $(function() {
        ...
        myDropzone.on('removedfile', function (files) {
            $.easyAjax({
                url: "{{ route('listing.deleteFile') }}",
                type: "POST",
                container: ".dropzone",
                data: {file: files.name, '_token': "{{ csrf_token() }}" },
        ...
        function getRemoteFilesize(file_size)
        {
            var size = file_size;
            console.log(file_size);
            if (Number(file_size) < 1024)
                size = file_size + ' B'; 
            else if (Number(file_size) < 1048576)
                size = Math.round(Number(file_size) / 1024, 2) + ' KB'; 
            else if (Number(file_size) < 1073741824)
                size = Math.round(Number(file_size) / 1048576, 2) + ' MB'; 
            else if (Number(file_size) < 1099511627776)
                size = Math.round(Number(file_size) / 1073741824, 2) + ' GB'; 
            console.log(size);
            return size;
        ...
        @if($file->file_format != 'pdf')
            myDropzone.options.thumbnail.call(myDropzone, file, "{{ asset('storage/listing-files/'.$file->file_name .'.'.$file->file_format) }}");
        @endif
        // myDropzone.options.removedfile.call(file);
...
//    }
    ...
    function submitForm() {
        $.easyAjax({
            url: "{!! route('user.listing.update', $listing->id) !!}",
            type: "POST",
