        ...
        <div class="margin-top-0">
            ...
            <div class="js-accordion">
                <div class="js-accordion-item">
                    <div class="accordion-custom-btn js-accordion-header">View Timesheet & Milestones  <i class="icon-feather-plus"></i></div>
                    ...
                    <div class="row-group accordion-body js-accordion-body">
                            ...
                            @forelse($jobDetail->budgetDetails as $budgetDetail)
                                <div class="row">
                                    <div class="col-6">
                                        ...
                                        @foreach($budgetDetail->increase_history as $increase)
                                            <a href="#">{{ $increase->reason }} (#{{ $increase->id }})</a><br>
                                        @endforeach
                                    </div>
                                    <div class="col-2">
                                        <div class="modal-sub-">${{ $budgetDetail->budget }}</div>
                                        @foreach($budgetDetail->increase_history as $increase)
                                            <div class="modal-sub-">${{ round($increase->amount) }}</div>
                                        @endforeach
                                    </div>
                                    <div class="col-2">
                        ...
                        <div class="row margin-top-10">
                            ...
                            @if($showPaymentButton == 1)
                                <div class="col-6">
                                    <a href="javascript:;" onclick="showReleasePayment({{ $releasePaymentShift->budget_id }})" id="payment-release" class="popup-with-zoom-anim button button-sliding-icon ripple-effect modal-button">Release Payment <i class="icon-material-outline-arrow-right-alt"></i></a>
                                </div>
                            @else
                                <div class="col-6">
                            ...
                            <div class="col-6">
                                <div class="modal-note-release">Payment release: <span>{{ $releasePaymentTimer ? $releasePaymentTimer : '48:00' }}</span> <i class="icon-feather-info" title="Work must be verified within 2 days or payment will release" data-tippy-placement="top"></i></div>
                            </div>
...
<script>
    ...
    function showReleasePayment(budgetId) {
        var url = "{{ route('listing.show-release-payment',[':id']) }}";
