...
class ListingController extends BaseController
{
    ...
    public function index()
    {
        ...
        $this->listings = Listing::select(['listings.*'])->with(['user', 'files', 'dispute', 'budgetDetails' => function($q) {
                $q->with('shift');
            }, 'bookmark',
            'offer' => function($q) {
        ...
        $this->assignedListings = Listing::select(['listings.*', 'offers.user_id as offer_user_id'])
            ...
            ->distinct()
            ->get();
            // dd($this->assignedListings);

        $this->endedListings = Listing::select(['listings.id','listings.user_id','listings.job_title','listings.city','listings.state','listings.job_location'])
        ...
        $this->myBids = Listing::select(['listings.*'])
            ...
            ->distinct()
            ->get();
            // dd($this->myBids->last()->selected_offer);

        $this->completed = Listing::select(['listings.*'])
            ->with(['user', 'files', 'dispute', 'budgetDetails'  => function($q) {
                $q->with('shift');
            }, 'bookmark',
            'offer' => function($q) {
    ...
    public function store(CreateRequest $request)
    {
        ...
        return Reply::redirect(route('user.listing.index'), __('messages.listingCreated'), $listing->id);
    }

    public function uploadFiles(Request $request)
    {
        if(count($request->file) > 0){
            // $listing_file = ListingFile::where('listing_id', $request->listingID)->get();
            // if(!is_null($listing_file)){
            //     ListingFile::where('listing_id', $request->listingID)->delete();
            // }
    ...
    public function deleteFile(Request $request)
    {
        if($request->file !== null){
            ...
            $fileFormat = $file_temp[1];
            if(Storage::exists('listing-files/'.$file)){
                Storage::delete('listing-files/'.$file);
            }
            $listing_file = ListingFile::where('file_name', $fileName)->where('file_format', $fileFormat)->get();
            if(!is_null($listing_file)){
                ListingFile::where('file_name', $fileName)->where('file_format', $fileFormat)->delete();
            }
        }
    ...
    {
        $this->offer = Offer::with(['user', 'listing'])->where('listing_id', $listing_id)->where('user_id', $user_id)->first();
    ...
    {
        $this->budgetDetail = ListingBudget::findOrFail($budget_id);
        $this->budgetDetail->status = 'completed';
